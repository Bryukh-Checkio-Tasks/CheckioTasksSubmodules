...
def generate_threats():
    ...
    for i, r in enumerate(ROWS):
        for j, c in enumerate(COLS):
            th = []
            th.extend([c + ROWS[k] for k in range(8)])
            th.extend([COLS[k] + r for k in range(8)])
            th.extend([COLS[k] + ROWS[i - j + k] for k in range(8) if 0 <= i - j + k < 8])
            th.extend([COLS[k] + ROWS[- k + i + j] for k in range(8) if 0 <= - k + i + j < 8])
            threats[c + r] = sorted(set(th))
    for k, v in sorted(threats.items()):
        print(k, v)
    print(threats)
...
def place_queens(queens, free_cells=None):
    if len(queens) == 8:
        return queens
    if free_cells is None:
        free_cells = set(c + r for r in ROWS for c in COLS)
    attacked = set()
    for q in queens:
        attacked.update(q[0] + r for r in ROWS)
        attacked.update(c + q[1] for c in COLS)
        row = ROWS.index(q[1])
        col = COLS.index(q[0])
        attacked.update(COLS[k] + ROWS[row - col + k] for k in range(8) if 0 <= row - col + k < 8)
        attacked.update(COLS[k] + ROWS[- k + row + col] for k in range(8) if 0 <= - k + row + col < 8)
        attacked.remove(q)
    if attacked.intersection(queens):
        return set()
    attacked.update(queens)
    free_cells.difference_update(attacked)
    for free in free_cells:
        res = place_queens(queens.union({free}), free_cells.copy())
        if res:
            return res
    return set()

# place_queens({"b2", "c4", "d6", "e8"})
# place_queens({"a1", "h8"})
#
